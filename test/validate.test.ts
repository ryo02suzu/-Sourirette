...
test("バリデータ: 無効な傷病名コードを検出（コード2016・オプション検証）", () => {
  const records = sampleFile(); // HS の傷病名コードは 5250001
  const known = (code: string) => code === "5250001" || code === "0000999";
  assert.deepEqual(validateUkeRecords(records, { isKnownDiseaseCode: known }), []);
  // 架空コードに差し替えると検出
  const hs = records.find((r) => r.identifier === "HS")!;
  hs.fields[3] = "9999999";
  const issues = validateUkeRecords(records, { isKnownDiseaseCode: known });
  assert.ok(issues.some((i) => i.code === "2016"));
});
